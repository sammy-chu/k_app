...
async function analyzeAMDTrades() {
  try {
    ...
    console.log(`=== AMD.BL 在 11:15 分钟内的交易数据分析 ===`);
    console.log(`总交易记录数: ${trades.length}`);
    
    if (trades.length === 0) {
      console.log('该分钟内没有找到符合条件的交易记录');
      return;
    }
    ...
    console.log(`百分比幅度: ${percentageAmplitude.toFixed(4)}%`);
    ...
    console.log(`涨跌幅: ${priceChangePercent.toFixed(4)}%`);
